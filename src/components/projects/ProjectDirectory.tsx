...
export function ProjectDirectory({ locale }: { locale: string }) {
  ...
  return <section className="directory-list" aria-label="Project directory">
    ...
    <p className="directory-count" role="status">{visible.length} {visible.length === 1 ? 'project' : 'projects'}{query.trim() ? ` matching “${query.trim()}”` : ' to explore'} · Featured projects first</p>
